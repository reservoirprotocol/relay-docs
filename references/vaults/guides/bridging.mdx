---
title: Bridging
description: In this guide, we will learn how to successfully bridge funds from an origin to a vault.
sidebarTitle: Bridging
---

## Indentifying the vault and origin

The first step is to identify the vault you want to bridge to. This is based on the currency and chain on which you want to receive the funds. You can chose to _harcode_ trusted vault addresses (like the ones deployed by the Relay team), or use our [Backend API](../backend.mdx) to query the vaults available on a specific chain and currency.
